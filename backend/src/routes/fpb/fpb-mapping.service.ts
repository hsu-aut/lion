...
import { AlternativeFlow, Energy, Flow, FlowElement, Information, 
	ParallelFlow, Process, ProcessOperator, Product, Project, 
	Relation, SystemBoundary, TechnicalResource, Usage } from './fpb.model';

@Injectable()
export class FpbMappingService {
     ...
     */
	prependPrefixes(triples: string): string {
	...
	createFlowObject(flowObject): string {

		let flowClass;
		const flowID = '<urn:' + flowObject.id + ">";

		if (flowObject instanceof Product) {
			flowClass = flowID + " a VDI3682:Product.\n";
		} else if (flowObject instanceof Energy) {
			flowClass = flowID + " a VDI3682:Energy.\n";
		} else if (flowObject instanceof Information) {
			flowClass = flowID + " a VDI3682:Information.\n";
		} else if (flowObject instanceof TechnicalResource) {
			flowClass = flowID + " a VDI3682:TechnicalResource.\n";
		} else if (flowObject instanceof ProcessOperator) {
		...
		if (flowObject instanceof (Product || Energy || Information)) {
	...
	createRelations(relationObject, statesArray): string {
		...
		if (relationObject instanceof ParallelFlow) {

			if (sourceObject instanceof (Product || Information || Energy)) {
				triples = triples + `${domainId} VDI3682:isParallelInputFor ${rangeId}.\n`;
			} else if (sourceObject instanceof ProcessOperator) {
				triples = triples + `${domainId} VDI3682:hasParallelOutput ${rangeId}.\n`;
			}

		} else if (relationObject instanceof AlternativeFlow) {
	...
	createProcess(processObject): string {
	...
	mapJsonToRDF(fpbjsJSON) {

		// deserialize fpbjs
		const model = this.buildModel(fpbjsJSON);

		// build ttl string
		let rdf = this.buildRDF(model);

		// remove duplicates 
		rdf = this.removeDuplicates(rdf);

		// prepend prefixes
		rdf = this.prependPrefixes(rdf);
	...
	buildRDF(fpbjsModel) {
		...
		for (let i = 0; i < fpbjsModel.process.length; i++) {
			...
			rdf = rdf + this.createProcess(fpbjsModel.process[i]);

			// build states
			fpbjsModel.process[i].states.forEach(state => {
				rdf = rdf + this.createFlowObject(state);
			});
			// build processOperators
			fpbjsModel.process[i].processOperators.forEach(processOperator => {
				rdf = rdf + this.createFlowObject(processOperator);
			});
			// build technical resources
			fpbjsModel.process[i].resources.forEach(resource => {
				rdf = rdf + this.createFlowObject(resource);
			});

			// add relations
			rdf = rdf + this.createRelations(fpbjsModel.process[i].relations, fpbjsModel.process[i].states);
	...
	buildModel(fpbjsJSON) {

		// initialize model
		const fileProjectInfo = fpbjsJSON.find(object => object.$type == "fpb:Project");
		const model = new Project(fileProjectInfo.targetNamespace + "/" + fileProjectInfo.name, fileProjectInfo.name);
		...
		for (let i = 0; i < fpbjsJSON.length; i++) {
			...
			if (fpbjsJSON[i].hasOwnProperty("process")) {
				processArray.push(this.buildProcess(fpbjsJSON[i]));
			}
		}
	...
	buildProcess(fpbjsJson_Process_Object) {

		// create new process
		const process = new Process(fpbjsJson_Process_Object.process.id, fpbjsJson_Process_Object.process.name);

		// add system boundary
		process.systemBoundary = new SystemBoundary(fpbjsJson_Process_Object.process.consistsOfSystemLimit, fpbjsJson_Process_Object.process.name);
		...
		fpbjsJson_Process_Object.elementDataInformation.forEach(fileElement => {
			...
			case "fpb:Product":
				element = new Product(fileElement.id, fileElement.identification.shortName);
				break;
			case "fpb:Energy":
				element = new Energy(fileElement.id, fileElement.identification.shortName);
				break;
			case "fpb:Information":
				element = new Information(fileElement.id, fileElement.identification.shortName);
				break;
			case "fpb:ProcessOperator":
				element = new ProcessOperator(fileElement.id, fileElement.identification.shortName);
				break;
			case "fpb:TechnicalResource":
				element = new TechnicalResource(fileElement.id, fileElement.identification.shortName);
				break;
			case "fpb:Flow":
				element = new Flow(fileElement.id, "");
				break;
			case "fpb:Usage":
				element = new Usage(fileElement.id, "");
				break;
			case "fpb:ParallelFlow":
				element = new ParallelFlow(fileElement.id, "");
				break;
			case "fpb:AlternativeFlow":
				element = new AlternativeFlow(fileElement.id, "");
				break;
			default:
				break;
			}
			...
			if (element instanceof ParallelFlow) {
				element.inTandemWith = fileElement.inTandemWith;
			}

			if (element instanceof (Product || Information || Energy)) {
				process.states.push(element);
			} else if (element instanceof ProcessOperator) {
				process.processOperators.push(element);
			} else if (element instanceof TechnicalResource) {
