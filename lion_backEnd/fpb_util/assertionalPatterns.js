...
function createFlowObject(flowObject) {
    ...
    if (flowObject instanceof (fpb.Product || fpb.Energy || fpb.Information)) {
        flowObject.incomingObjects.forEach(incomingObject => {
            let incomingObjectId = '<urn:' + incomingObject + ">";
    ...
    let triples = flowClass + commonTriples + flowTriples;
...
function createRelations(relationObject, statesArray) {
    ...
    let domainId = "<urn:" + relationObject.source + ">";
    let rangeId = "<urn:" + relationObject.target + ">";

    let sourceObject = statesArray.find(stateObject => relationObject.source == stateObject.id)
...
function createProcess(processObject) {
    ...
    let systemBoundaryId = "<urn:" + processObject.systemBoundary.id + ">";

    let systemBoundaryTriple = systemBoundaryId + " a VDI3682:SystemBorder.\n";

    let triples = systemBoundaryTriple;

    // put states on boarder
    for (let i = 0; i < processObject.systemBoundary.onBoarder.length; i++) {
        let onBoarderTriple = '<urn:' + processObject.systemBoundary.onBoarder[i] + ">" + ' VDI3682:isOnSystemBorder ' + systemBoundaryId + ".\n"
    ...
    if (processObject.isDecomposedProcessOperator != null) {
        let upperProcessId = "<urn:" + processObject.isDecomposedProcessOperator + ">";
        let compositionTriple = `${upperProcessId} VDI3682:hasUpperProcess ${systemBoundaryId}.\n`;
    ...
    processObject.processOperators.forEach(processOperator => {
        let processOperatorId = "<urn:" + processOperator.id + ">";
        let withinTriple = `${processOperatorId} VDI3682:isWithinSystemBorder ${systemBoundaryId}.\n`;
    ...
    processObject.states.forEach(state => {
        if (processObject.systemBoundary.onBoarder.findIndex(id => state.id == id) == -1) {
            let stateId = "<urn:" + state.id + ">";
            let withinTriple = `${stateId} VDI3682:isWithinSystemBorder ${systemBoundaryId}.\n`;
            triples = triples + withinTriple;
        }
    })
...
function mapJsonToRDF(fpbjsJSON) {
    ...
    // deserialize fpbjs
    let model = buildModel(fpbjsJSON);
...
function buildModel(fpbjsJSON) {
    ...
    // initialize model
    let fileProjectInfo = fpbjsJSON.find(object => object.$type == "fpb:Project")
...
function buildProcess(fpbjsJson_Process_Object) {
    ...
    // create new process
    let process = new fpb.Process(fpbjsJson_Process_Object.process.id, fpbjsJson_Process_Object.process.name)
